...
public class AlumnoDAO extends BaseDAO {
	...
	public Alumno insertar(Alumno alumno) throws DAOExcepcion {
		...
		try {
			...
			/*
			 * int id = 0; query = "select last_insert_id()"; stmt =
			 * con.prepareStatement(query); rs = stmt.executeQuery(); if
	...
	public Alumno insertarActualizar(Alumno alumno) throws DAOExcepcion {
		...
		String queryMerge = "MERGE INTO alumno a "
				...
				+ "ON (TRIM(a.a_codigo) = alumno.a_codigo ) " + "WHEN MATCHED THEN "
	...
	public List<AlumnoBO> listarAlumnoDisponibilidad(String codCurso) {
		...
		List<AlumnoBO> lsAlumnos = new ArrayList<>();
		PreparedStatement pstm = null;
		
		//OBTENER ANIO PERIODO ACTUAL		
		String sql = "SELECT ANIO, PERIODO FROM CICLO WHERE ACTIVO = 1";
		String anio = "" , periodo  ="";
		...
		try {
			...
			System.out.println(" anio " + anio + " periodo " + periodo);
			
			sql = "SELECT DISTINCT UPPER(TRIM(A.NOM_ALUMNO)) NOMBRE,  TD.COD_USUARIO CODIGO "
					+ " FROM TABLA_DISPONIBILIDAD TD, ALUMNO_REGULAR A"
					+ " WHERE TD.ANIO  = ? "
					+ " AND TD.PERIODO = ? "
					+ " AND TD.COD_CURSO = ?"
					+ " AND TD.COD_USUARIO = A.COD_ALUMNO "
					+ " AND TD.TIPO_USUARIO = 2";

			pstm = con.prepareStatement(sql);
			pstm.setString(1, anio);
			pstm.setString(2, periodo);
			pstm.setString(3, codCurso);
			
			rs = pstm.executeQuery();			
			
			while(rs.next()){
				AlumnoBO alumnoBO = new AlumnoBO();
				alumnoBO.setaCodigo(rs.getString("CODIGO"));
				alumnoBO.setaNombre(rs.getString("NOMBRE"));
				lsAlumnos.add(alumnoBO);
			}
