...
BEGIN
  IF PROCESO_TUTORIA     = 1 THEN
    OPEN C_ROL FOR SELECT ID_ROL, DESC_ROL FROM ROL WHERE ID_ROL BETWEEN 1 AND 5 OR ID_ROL = 12 ORDER BY ID_ROL;
  ELSE
    IF PROCESO_TUTORIA   = 2 THEN
      OPEN C_ROL FOR SELECT ID_ROL, DESC_ROL FROM ROL WHERE ID_ROL BETWEEN 6 AND 11 OR ID_ROL = 13 ORDER BY ID_ROL;
    ELSE
      IF PROCESO_TUTORIA     = 3 THEN
        OPEN C_ROL FOR SELECT ID_ROL, DESC_ROL FROM ROL WHERE ID_ROL BETWEEN 14 AND 18;
      END IF;
    END IF;
    END IF;
  END BUSCAR_ROL;
